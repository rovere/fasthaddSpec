...
%if %{with python}
pushd python
python ./setup.py install --root=%{_buildrootdir} --single-version-externally-managed --record=INSTALLED_FILES --optimize=1
popd
%endif
# install -p -m 644 -D %{SOURCE1} %{_buildrootdir}%{_datadir}/vim/vimfiles/ftdetect/proto.vim
# install -p -m 644 -D editors/proto.vim %{_buildrootdir}%{_datadir}/vim/vimfiles/syntax/proto.vim

%if %{with java}
pushd java
install -d -m 755 %{_buildrootdir}%{_javadir}
install -pm 644 target/%{name}-java-%{version}.jar %{_buildrootdir}%{_javadir}/%{name}.jar

install -d -m 755 %{_buildrootdir}%{_javadocdir}/%{name}
cp -rp target/site/apidocs %{_buildrootdir}%{_javadocdir}/%{name}

install -d -m 755 %{_buildrootdir}%{_mavenpomdir}
install -pm 644 pom.xml %{_buildrootdir}%{_mavenpomdir}/JPP-%{name}.pom
